...
public class MongoBookDAOImp implements BookDAO{
	...
	public MongoBookDAOImp() {
		asyncBooksCollection = MongoDAOFactory.getInstance().getAsyncDB().getCollection( BOOKS_COLLECTION );
		syncBooksCollection = MongoDAOFactory.getInstance().getSyncDB().getCollection( BOOKS_COLLECTION );
	}
